document.addEventListener('DOMContentLoaded',()=>{
  ...
  const check=async()=>{if(!button)return;
  ...
  try{
    ...
    if(data.available){button.classList.add('update-available');button.textContent='Update beschikbaar';document.querySelector('#install-update-form')?.removeAttribute('hidden');}
  ...
  check();document.querySelector('#check-updates')?.addEventListener('click',check);
});
