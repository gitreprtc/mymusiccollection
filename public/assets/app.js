...
async function checkOwned(barcode) { const output=$('#scan-result');output.textContent='Albumgegevens opzoeken in MusicBrainz…';try{const r=await fetch('?action=check&barcode='+encodeURIComponent(barcode));const d=await r.json();if(d.found){output.innerHTML='✅ Je hebt deze: <strong>'+d.artist+' — '+d.title+'</strong> ('+d.format+').';return;}const suggestion=d.release?'MusicBrainz vindt: '+d.release.artist+' — '+d.release.title+' ('+d.release.format+(d.release.year?', '+d.release.year:'')+').':'MusicBrainz vond geen betrouwbare albumgegevens.';const yes=window.confirm(suggestion+'\n\nDeze barcode staat nog niet in je collectie. Wil je dit album toevoegen?');if(yes){addScannedRecord(barcode);}else{output.textContent='Niet toegevoegd aan je collectie.';}}catch(error){output.textContent='Albumgegevens konden niet worden opgehaald. Probeer opnieuw.';} }
