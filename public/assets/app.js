...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  if(titleField){
    ...
    const parent=titleField.parentElement,status=document.createElement('small'),form=titleField.closest('form'),selectedSource=document.createElement('input'),selectedId=document.createElement('input');selectedSource.type=selectedId.type='hidden';selectedSource.name='suggestion_source';selectedId.name='suggestion_id';form?.append(selectedSource,selectedId);parent.style.position='relative';parent.append(panel,status);Object.assign(status.style,{position:'absolute',right:'12px',top:'50%',transform:'translateY(4px)',color:'#6d6870',pointerEvents:'none'});new MutationObserver(()=>panel.querySelectorAll('button:not([data-labelled])').forEach(button=>{const main=button.querySelector('strong'),parts=(main?.textContent||'').split(' — ');if(parts.length<2)return;button.dataset.labelled='1';const fields=document.createElement('div');fields.style.cssText='display:grid;grid-template-columns:1fr 1fr;gap:10px';for(const [label,value] of [['ARTIEST',parts[0]],['TITEL',parts.slice(1).join(' — ')]]){const field=document.createElement('span'),heading=document.createElement('small'),text=document.createElement('strong');heading.textContent=label;heading.style.cssText='display:block;font-size:.68rem;letter-spacing:.06em;color:#756e75';text.textContent=value;field.append(heading,text);fields.append(field);}main.replaceWith(fields);})).observe(panel,{childList:true});let searchTimer,lastQuery='',searchController;
