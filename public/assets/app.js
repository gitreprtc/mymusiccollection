...
async function checkOwned(barcode) { const r=await fetch(`?action=check&barcode=${encodeURIComponent(barcode)}`); const d=await r.json(); if(d.found){$('#scan-result').innerHTML=`✅ Je hebt deze: <strong>${d.artist} — ${d.title}</strong> (${d.format}).`;return;}const yes=window.confirm('Deze barcode staat nog niet in je collectie. Wil je dit album toevoegen?');if(yes){addScannedRecord(barcode);}else{$('#scan-result').textContent='Niet toegevoegd aan je collectie.';} }
