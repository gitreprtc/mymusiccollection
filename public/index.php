...
if (($_GET['action'] ?? '') === 'vision-barcode') { requireLogin(); header('Content-Type: application/json'); checkCsrf(); try { if (empty($_FILES['barcode_photo']['tmp_name']) || $_FILES['barcode_photo']['error'] !== UPLOAD_ERR_OK || $_FILES['barcode_photo']['size'] > 10 * 1024 * 1024) throw new RuntimeException('Maak eerst een barcodefoto van maximaal 10 MB.'); echo json_encode(['barcode' => VisionBarcode::read($_FILES['barcode_photo']['tmp_name'])]); } catch (Throwable $e) { http_response_code(422); echo json_encode(['error' => $e->getMessage()]); } exit; }
