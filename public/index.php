...
function layout(string $title, string $content): never {
    ...
    $nav=$logged ? '<nav><a class="'.($page==='dashboard'?'active':'').'" href="?page=dashboard">Collectie</a><a class="'.($page==='add'?'active':'').'" href="?page=add">Toevoegen</a><a class="'.($page==='scan'?'active':'').'" href="?page=scan">Scannen</a><a class="'.($page==='integrations'?'active':'').'" href="?page=integrations">Integraties</a><a href="?action=logout">Uitloggen</a></nav>' : '';
    ...
    $content.='<link rel="stylesheet" href="assets/collection-list.css?v='.$assetVersion.'"><script src="assets/html5-qrcode.min.js?v='.$assetVersion.'"></script><script src="assets/update.js?v='.$assetVersion.'" defer></script><script src="assets/barcode-photo.js?v='.$assetVersion.'" defer></script><script src="assets/direct-scanner.js?v='.$assetVersion.'" defer></script><script src="assets/add-live-barcode.js?v='.$assetVersion.'" defer></script><script src="assets/rear-camera-capture.js?v='.$assetVersion.'" defer></script>';
...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    try {
        ...
        if ($form==='record') { $artist=trim($_POST['artist']??'');$title=trim($_POST['title']??'');$format=$_POST['format']??'';if(!$artist||!$title||!in_array($format,['CD','LP'],true)) throw new RuntimeException('Artiest, titel en type zijn verplicht.'); $cover=imageUpload('cover');$back=imageUpload('back');$barcodePhoto=imageUpload('barcode_photo');$scanPhoto=$_POST['scan_photo']??'';if(!$barcodePhoto && preg_match('#^scans/[a-f0-9]{40}\.(jpg|png|webp)$#',$scanPhoto) && is_file(UPLOADS.'/'.$scanPhoto))$barcodePhoto=$scanPhoto; db()->prepare('INSERT INTO records(artist,title,format,barcode,tracklist,cover_path,back_path,barcode_path,user_id) VALUES(?,?,?,?,?,?,?,?,?)')->execute([$artist,$title,$format,trim($_POST['barcode']??''),trim($_POST['tracklist']??''),$cover,$back,$barcodePhoto,$_SESSION['user_id']]);flash('Toegevoegd aan je collectie.');redirect('?page=dashboard'); }
